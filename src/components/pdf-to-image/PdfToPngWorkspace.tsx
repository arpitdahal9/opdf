...
export function PdfToPngWorkspace() {
  ...
  async function downloadPageAsPng(pageIndex: number) {
    if (!pdfBytes) return;
    ...
    try {
      ...
      downloadDataUrl(dataUrl, `${fileName}-page-${pageIndex + 1}.png`);
    } catch (err) {
      setError(getFriendlyPdfError(err));
    } finally {
  ...
  async function downloadAllAsPng() {
    ...
    try {
      ...
      for (let i = 0; i < pageCount; i += 1) {
        ...
        zip.file(`${fileName}-page-${i + 1}.png`, bytes);
    ...
    return (
      <div className="page-wrap animate-fadeIn space-y-6">
        <section className="space-y-2">
          <h1 className="text-3xl font-bold">PDF to PNG</h1>
          <p className="text-gray-600 dark:text-slate-300">
            Convert each PDF page to a PNG image. No upload — everything runs in your browser.
          </p>
        </section>
        {error ? (
  ...
  return (
    <div className="page-wrap animate-fadeIn space-y-6">
      <section className="space-y-2">
        <h1 className="text-3xl font-bold">PDF to PNG</h1>
        <p className="text-gray-600 dark:text-slate-300">
          {pageCount} {pageCount === 1 ? "page" : "pages"}. Download individual pages or the entire PDF as PNG images.
        </p>
      </section>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Downloaded images will be saved as <code className="font-mono text-xs">{fileName}-page-N.png</code>.
        </p>
        <button
          type="button"
          className="primary-button"
          disabled={isDownloading}
          onClick={() => void downloadAllAsPng()}
        >
          {downloadingAll ? "Downloading all pages…" : "Download all pages as PNG"}
        </button>
      </div>
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2 p-3 border-t border-border dark:border-gray-700">
              ...
              <button
                ...
                disabled={isDownloading}
                onClick={() => downloadPageAsPng(i)}
              >
                <Download className="h-4 w-4" />
                {downloadingPage === i ? "Converting…" : "Download as PNG"}
              </button>
            </div>
          </div>
        ))}
        ...
        <ProcessingSpinner
          label={
            downloadingAll && bulkProgress !== null
              ? `Converting PDF to PNGs… (${bulkProgress}%)`
              : "Converting page to PNG…"
          }
        />
      ) : null}
